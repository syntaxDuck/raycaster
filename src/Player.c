...
void castPlayerRays(Player *player, Map map)
{
    ...
    double rad_per_col = player->actor.field_of_view / win_ctx->width; // Radians per pixel
    ...
    for (int x = 0; x < win_ctx->width; x++)
    {
        ...
        camera_x = 2 * x / (double)win_ctx->width - 1; // Normalized coordinate in camera space
